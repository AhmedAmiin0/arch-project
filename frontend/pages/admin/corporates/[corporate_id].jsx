...
const EditCorporate = ({ corporate }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async values => {
      ...
      if (values.corporate_logo && typeof values.corporate_logo === 'object' && values.corporate_logo.size > 0) {
  ...
  return <ContentPageContainer>
    <form onSubmit={formik.handleSubmit} ref={formRef}>
      <Typography variant={'h4'} mb={3}>Edit Corporate</Typography>
      <ContentPageFlexBox>
        ...
        <Box flex={3} display={'flex'} flexDirection={'column'} justifyContent={'center'} alignItems={'center'}>
          ...
          <label htmlFor="contained-button-file" style={{
            ...
            <Input accept="image/*" id="contained-button-file" type="file" sx={{ display: 'none', }}
              name={'corporate_logo'}
              onChange={(e) => formik.setFieldValue('corporate_logo', e.target.files[0])}
            />
            {formik.values.corporate_logo?.src?.length != 0 && <img
              src={formik.values.corporate_logo.src || URL.createObjectURL(formik.values.corporate_logo)}
