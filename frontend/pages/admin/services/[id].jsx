...
const Edit = ({ service }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async values => {
      ...
      if (values.service_thumb && typeof values.service_thumb === 'object' && values.service_thumb.size > 0) {
        formData.append('service_thumb', values.service_thumb);
      }
      formData.append('_method', 'PUT');
      try {
        let res = await axios.post('services/' + id, formData, { headers: { 'Accept-Language': locale } })
        console.log(res)
  ...
  return <ContentPageContainer>
    <form onSubmit={formik.handleSubmit} ref={formRef}>
      <Typography variant={'h4'} mb={3}>Edit Service</Typography>
      <ContentPageFlexBox>
        ...
        <Box flex={3} display={'flex'} flexDirection={'column'} justifyContent={'center'} alignItems={'center'}>
          ...
          />
          <TextField
            label={'Subtitle'} fullWidth sx={{ mb: 3 }}
            name={'subtitle'}
            {...formik.getFieldProps('subtitle')}
            error={formik.touched.subtitle && formik.errors.subtitle ? true : false}
            helperText={formik.touched.subtitle && formik.errors.subtitle ? formik.errors.subtitle : ''}

          />
          <TextField
            label={'Excerpt'} fullWidth sx={{ mb: 3 }} multiline={true} rows={5}
            name={'excerpt'}
            {...formik.getFieldProps('excerpt')}
            error={formik.touched.excerpt && formik.errors.excerpt ? true : false}
            helperText={formik.touched.excerpt && formik.errors.excerpt ? formik.errors.excerpt : ''}

          />

          <TextField
            label={'Description'} fullWidth sx={{ mb: 3 }} multiline={true} rows={10}
            name={'description'}
...
export async function getServerSideProps({ params, locale }) {
  const { id } = params;
  const service = await axios.get('/services/' + id, {
