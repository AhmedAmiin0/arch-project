...
export const useDelete = (locale) => {
  ...
  const deleteItem = async (url,id = null) => {
    try {
      await axios.delete(`${url}/${id}`, {headers: {'Accept-Language': locale}});
...
export const useSearch = (locale) => {
  const search = async (url, e,limit=5,page=1) => {
    let res = await axios.get(`search/${url}?q=${e.target.value}&limit=${limit}&page=${page}`, {headers: {'Accept-Language': locale}})
    if (res.status === 200) {
      return res
    }
    console.log(res);
...
export const useCreate = (locale,url) => {
  ...
  const createItem = async (data) => {
    try {
      let res = await axios.post(`${url}`, data, {headers: {'Accept-Language': locale}});
